...
void push_button_up_driver_init(void)
{
	...
	GPIO_PORTE_IS_R    &= ~(1<<1);      /* PD2 detect edges */
  GPIO_PORTE_IBE_R   &= ~(1<<1);      /* PD2 will detect a certain edge */
  GPIO_PORTE_IEV_R   &= ~(1<<1);      /* PD2 will detect a falling edge */
  GPIO_PORTE_ICR_R   |= (1<<1);       /* Clear Trigger flag for PD2 (Interupt Flag) */
  GPIO_PORTE_IM_R    |= (1<<1);       /* Enable Interrupt on PD2 pin */
