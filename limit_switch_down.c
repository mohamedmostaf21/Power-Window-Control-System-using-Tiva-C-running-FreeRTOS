...
void limit_switch_down_init(void)
{
	...
	GPIO_PORTA_IS_R    &= ~(1<<6);      /* PD2 detect edges */
  GPIO_PORTA_IBE_R   &= ~(1<<6);      /* PD2 will detect a certain edge */
  GPIO_PORTA_IEV_R   &= ~(1<<6);      /* PD2 will detect a falling edge */
  GPIO_PORTA_ICR_R   |= (1<<6);       /* Clear Trigger flag for PD2 (Interupt Flag) */
  GPIO_PORTA_IM_R    |= (1<<6);       /* Enable Interrupt on PD2 pin */
